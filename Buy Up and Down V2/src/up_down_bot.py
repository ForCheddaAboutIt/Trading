...
Strategy: 
1. Skips first market of session (Configurable).
...
class AverageDownBot:
    ...
    def __init__(self, settings):
        ...
        # --- NEW: Reversal & Price Settings ---
        self.entry_max_price = float(os.getenv("STRATEGY_ENTRY_MAX_PRICE", 0.30))
        self.rsi_entry_min = float(os.getenv("STRATEGY_RSI_ENTRY_MIN", 30.0))
        self.bb_pct_b_entry_min = float(os.getenv("STRATEGY_BB_PCT_B_ENTRY_MIN", -0.1))
        ...
        # We only disable if we are IN the first market AND the user wants to skip it
    ...
    async def process_strategy(self, side_key: str, book: dict):
        """
        Modified Strategy: Cheap Reversals Only.
        - Enters only if Price < Max (0.30).
        - Enters only if RSI > Min (30) and %B > Min (-0.1).
        - STILL requires MFI/Stoch exhaustion.
        - NEVER SELLS (Holds to settlement).
        """
        st = self.strategies[side_key]
        ...
        rsi = inds.get("RSI", 50.0)             # Needed for new check
        bb_pct_b = inds.get("BB_PctB", 0.5)     # Needed for new check
        
        # Extract Best Prices
        best_ask = book.get("best_ask")
        if best_ask is None: return
        ...
        if can_buy:
            # --- NEW FILTERS ---
            # 1. Price is cheap enough?
            price_ok = (best_ask <= self.entry_max_price)
            
            # 2. Is it a reversal (safe) or a crash (unsafe)?
            # We want RSI to be 'recovering' (not < 25) and Price not too far below bands.
            rsi_ok = (rsi >= self.rsi_entry_min)
            bb_ok = (bb_pct_b >= self.bb_pct_b_entry_min)
            
            # 3. Original Exhaustion Checks
            is_exhausted = (mfi <= self.mfi_entry_max) and (stoch_k <= self.stoch_entry_max)

            if price_ok and rsi_ok and bb_ok and is_exhausted:
                # Target "UP" only
                target_side = "UP" 
                ...
                if side_key == target_side:
                    ...
                    if buy_size > 0:
                        logger.info(f"🔵 ENTRY ({side_key}): Price ${best_ask:.2f} | RSI {rsi:.1f} | %B {bb_pct_b:.2f} | {desc}")
        ...
        # ==========================================
        # 2. EXIT LOGIC (DISABLED)
        # ==========================================
        # User requested: "I don't want the bot to ever leave the trade."
        # Logic removed. Bot will hold until Polymarket settles the position automatically.
        pass
