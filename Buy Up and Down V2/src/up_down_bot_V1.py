...
class AverageDownBot:
    ...
    async def process_strategy(self, side_key: str, book: dict):
        """
        Panic Exhaustion Strategy (No Time Stops).
        - Enters on MFI <= 8 and Stoch <= 20.
        - Exits ONLY on 'Panic' spikes (Stoch > 40 or MFI > 20).
        - Otherwise, holds to Settlement (Expiry).
        """
        st = self.strategies[side_key]
        ...
        if can_buy:
            ...
            if is_panic and is_exhausted:
                # Target "UP" based on Panic Exhaustion
                target_side = "UP" 
        ...
        # ==========================================
        # 2. EXIT LOGIC (Panic Spikes Only)
        # ==========================================
        if st.total_shares > 0:
            
            # We ONLY sell if indicators spike (taking profit/safety).
            # Otherwise, we hold to settlement.
            
            should_sell = False
            reason = ""
            
            # Condition: Stoch > 40 OR MFI >= 20
            if stoch_k > self.stoch_panic_exit:
                should_sell = True
                reason = f"Stoch Spike > {self.stoch_panic_exit}"
            elif mfi >= self.mfi_panic_exit:
                should_sell = True
                reason = f"MFI Spike >= {self.mfi_panic_exit}"
                
            if should_sell:
                logger.warning(f"🚨 PANIC EXIT ({side_key}): {reason}")
