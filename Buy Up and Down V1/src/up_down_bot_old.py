...
class AverageDownBot:
    ...
    def __init__(self, settings):
        ...
        # --- MFI Strategy Parameters (Loaded from ENV) ---
        # We use os.getenv directly so you don't have to edit config.py
    ...
    async def process_strategy(self, side_key: str, book: dict):
        """
        Advanced Strategy (Env Configured):
        1. ABORT if MFI >= MFI_ABORT_THRESHOLD.
        2. BUY UP only if MFI < MFI_BUY_UP_MAX.
        3. BUY DOWN only if MFI > MFI_BUY_DOWN_MIN.
        4. BLOCK PANIC SELL if MFI < MFI_PANIC_BLOCK_MAX.
        """
        st = self.strategies[side_key]
        ...
        # --- RULE 1: ABORT MARKET (Safety) ---
        if mfi >= self.mfi_abort_thresh:
            if st.total_shares > 0:
                logger.warning(f"🚨 ABORT SIGNAL (MFI {mfi:.1f} >= {self.mfi_abort_thresh}) - CLOSING {side_key} NOW!")
        ...
        # --- RULE 2 & 4: STRICT MFI ENTRY ---
        can_buy_mfi = False
        mfi_reason = ""
        
        if side_key == "UP":
            if mfi < self.mfi_buy_up_max:
                can_buy_mfi = True
            else:
                mfi_reason = f"(MFI {mfi:.1f} >= {self.mfi_buy_up_max})"
                
        elif side_key == "DOWN":
            # We only Buy DOWN if MFI is HIGH (Overbought)
            if mfi < self.mfi_buy_down_min:
                can_buy_mfi = True
            else:
                mfi_reason = f"(MFI {mfi:.1f} <= {self.mfi_buy_down_min})"

        buy_signal = False
        buy_size = 0.0
        reason = ""

        if can_buy_time and can_buy_price_floor and can_buy_mfi:
            ...
            if st.total_shares == 0:
                if best_ask <= self.entry_price:
                    ...
                    if buy_size > 0:
                        buy_signal = True
                        reason = f"Initial Entry ({side_key}) - MFI {mfi:.1f} OK"
            ...
            # Case B: Average Down
                ...
                if st.last_buy_price and best_ask <= (st.last_buy_price - self.drop_interval):
                    ...
                    if buy_size > 0:
                        buy_signal = True
                        reason = f"Avg Down - MFI {mfi:.1f} OK"
        ...
        # Log reason for blocking (DEBUG level only to avoid spam)
        elif can_buy_time and can_buy_price_floor and not can_buy_mfi:
             logger.debug(f"⛔ {side_key} Blocked by Strategy: {mfi_reason}")

        # --- 2. EXIT LOGIC (SELLING) ---
        if st.total_shares > 0:
            
            # --- RULE 3: PANIC SELL BLOCKER ---
            # --- RULE 3: PANIC SELL BLOCKER ---
            if best_bid <= 0.10 and not st.stop_loss_triggered:
                
                # FIX: Removed 'side_key == "UP"' so this now protects BOTH sides
                if mfi < self.mfi_panic_block_max:
                    # We are trapped, but MFI says we are oversold. HOLD.
                    logger.info(f"🛡️ BLOCKING PANIC SELL ({side_key}): MFI {mfi:.1f} < {self.mfi_panic_block_max}. Holding for bounce...")
                    return # Exit function, do not sell
                
                # Otherwise, proceed with panic sell
            ...
            if self.get_time_remaining() == "CLOSED":
                ...
                try:
                    ...
                    if new_market != self.market_slug:
                        ...
                        break  # Exit retry, proceed to WSS/loop
                    else:
                        break  # Same market, no switch needed
                except RuntimeError as e:
                    if "retry" in str(e).lower():
                        logger.info("No active market yet. Waiting 30s...")
