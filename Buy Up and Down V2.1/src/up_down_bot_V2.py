...
class AverageDownBot:
    ...
    async def process_strategy(self, side_key: str, book: dict):
        ...
        if st.total_shares > 0:
            ...
            # ------------------------------------
            # A. EMERGENCY SELL (Override All)
            # ------------------------------------
            # Condition: Stoch > 40 OR MFI >= 20
            ...
            # ------------------------------------
            # B. TAKE PROFIT (Partials)
            # ------------------------------------
            # Condition: PnL >= +25%
            if pnl_pct >= self.tp_trigger_pct and not st.taken_profit:
                sell_size = normalize_size(st.total_shares * 0.5) # Sell 50%
                if sell_size > 0:
            ...
            if time_held_mins >= self.t4_checkpoint_min and not st.checked_t4:
                ...
                # "Early Proof" if ANY are true:
                # 1. Price >= Entry + $0.02
                # 2. PnL >= 15%
                # 3. Stoch Improving (Momentum)
                has_proof = (
                ...
                if has_proof:
                    ...
                    # DE-RISK: Sell 50% of current holdings
            ...
            if time_held_mins >= self.t6_exit_min and not st.checked_t6:
                ...
                if is_working:
                    # Runner Management: Hold to Expiry.
                    # (If we haven't taken profit yet, we likely did above at the 25% check, or will now).
