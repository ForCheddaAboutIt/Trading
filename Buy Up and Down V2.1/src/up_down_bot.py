"""
Panic Exhaustion Bot for Bitcoin 15min markets.

Strategy: 
1. Skips first market of session.
2. Enters ONLY when MFI <= 8 AND Stoch K <= 20 (Exhaustion).
3. Dynamic Sizing: 5% of account. 100% size if Stoch<=10, 75% if Stoch<=20.
4. Exit: Time-based stop at T+6 mins. If not working, sell immediately.
"""
...
class PositionState:
    ...
    def add_buy(self, price, size, stoch_k):
        cost = price * size
...
class AverageDownBot:
    """Bot implementing the Panic Exhaustion Strategy."""
    ...
    def __init__(self, settings):
        ...
        # Env vars used directly to ensure strict adherence to new logic
        self.mfi_entry_max = float(os.getenv("STRATEGY_MFI_ENTRY_MAX", 8))
        self.stoch_entry_max = float(os.getenv("STRATEGY_STOCH_K_ENTRY_MAX", 20))
        ...
        self.stoch_full_size_max = float(os.getenv("STRATEGY_STOCH_K_FULL_SIZE", 10))
        self.size_reduction_mult = float(os.getenv("STRATEGY_SIZE_REDUCTION_MULTIPLIER", 0.75))
        
        self.time_stop_minutes = float(os.getenv("STRATEGY_TIME_STOP_MINUTES", 6))
        self.working_price_bump = float(os.getenv("STRATEGY_WORKING_PRICE_BUMP", 0.03))
        self.working_pnl_pct = float(os.getenv("STRATEGY_WORKING_PNL_PERCENT", 0.20))
        
        self.mfi_panic_exit = float(os.getenv("STRATEGY_MFI_PANIC_EXIT", 20))
        self.stoch_panic_exit = float(os.getenv("STRATEGY_STOCH_PANIC_EXIT", 40))
        self.mfi_abort_thresh = float(os.getenv("MFI_ABORT_THRESHOLD", 90))

        # --- Trailing Stop Settings ---
        self.trailing_stop_enabled = os.getenv("STRATEGY_TRAILING_STOP_ENABLED", "false").lower() == "true"
        self.trailing_stop_amount = float(os.getenv("STRATEGY_TRAILING_STOP_AMOUNT", 0.05))

        self.t4_checkpoint_min = 4.0      # Checkpoint 1 time
        self.t6_exit_min = 6.0            # Primary Exit time
        
        # Checkpoint 1 (T+4) Criteria
        self.t4_min_pnl_pct = 0.15        # 15%
        self.t4_price_bump = 0.02         # $0.02
        
        # Checkpoint 2 (T+6) Criteria
        self.t6_min_pnl_pct = 0.25        # 25%
        self.t6_price_bump = 0.03         # $0.03
        
        # Take Profit Trigger
        self.tp_trigger_pct = 0.25        # Sell 50% if PnL >= 25%
        ...
        # --- Global Filters ---
        # We need to track if this is the "First Market" of the session.
        # We'll use a class-variable or persistent tracking.
        ...
        # Flag to ensure One Trade Per Market (as per strategy)
    ...
    async def get_indicators(self, use_shared_client=True, force_update=False) -> dict:
        """Fetch data and calculate indicators (cached)."""
            ...
            try:
                ...
                if use_shared_client:
                     for base_url in endpoints:
                        try:
                            r1 = await client_to_use.get(f"{base_url}/api/v3/klines?symbol=BTCUSDT&interval=1m&limit=900", timeout=2.0)
                    ...
                    async with client_to_use as client:
                        for base_url in endpoints:
                            try:
                                r1 = await client.get(f"{base_url}/api/v3/klines?symbol=BTCUSDT&interval=1m&limit=900", timeout=2.0)
    ...
    async def execute_order(self, side_key, token_id, price, size, is_buy: bool, stoch_k=0.0):
        ...
        if self.settings.dry_run:
            logger.info(f"Pre-sell simbalance: {self.sim_balance:.2f} | cost: {cost:.2f} | isbuy: {is_buy}")
    ...
    async def process_strategy(self, side_key: str, book: dict):
        """
        Panic Exhaustion Strategy with Multi-Stage Exit Logic.
        """
        st = self.strategies[side_key]
        ...
        if can_buy:
            is_panic = (mfi <= self.mfi_entry_max)
            is_exhausted = (stoch_k <= self.stoch_entry_max)
            
            if is_panic and is_exhausted:
                # We target "UP" based on Panic Exhaustion (Oversold -> Reversal)
                target_side = "UP" 
                
                if side_key == target_side:
                    bal = self.cached_balance if not self.settings.dry_run else self.sim_balance
                    risk_amt = bal * self.risk_per_trade
                    
                    if stoch_k <= self.stoch_full_size_max:
                        final_value = risk_amt
                        desc = "100% Size (Stoch <= 10)"
                    else:
                        final_value = risk_amt * self.size_reduction_mult
                        desc = "75% Size (Stoch 10-20)"
                    
                    raw_size = final_value / best_ask
                    buy_size = normalize_size(raw_size, self.min_order_size)
                    
                    if buy_size > 0:
                        logger.info(f"🔵 ENTRY SIGNAL ({side_key}): MFI {mfi:.1f}, Stoch {stoch_k:.1f} | {desc}")
                        success = await self.execute_order(side_key, st.token_id, best_ask, buy_size, True, stoch_k=stoch_k)
                        if success:
                            st.add_buy(best_ask, buy_size, stoch_k)
                            self.has_traded_this_market = True

        # ==========================================
    async def process_strategy(self, side_key: str, book: dict):
        """
        Panic Exhaustion Strategy with Trailing Stop & Multi-Stage Exits.
        """
        st = self.strategies[side_key]
        minutes_remaining = self.get_time_remaining_minutes()
        ...
        if st.total_shares > 0:
            ...
            # --- A. UPDATE HIGH WATER MARK ---
            if best_bid > st.high_price_seen:
                st.high_price_seen = best_bid

            # --- CALCULATE LIVE PNL ---
            price_gain = best_bid - st.entry_price
            pnl_pct = (price_gain / st.entry_price) if st.entry_price > 0 else 0.0

            # ------------------------------------
            # B. ACTIVATED TRAILING STOP ($0.05)
            # ------------------------------------
            # Logic: Activate if Peak is >= Entry + $0.05. Sell if drops $0.05 from Peak.
            if self.trailing_stop_enabled:
                
                # Dynamic Activation: Ensure we are at least "Amount" in profit (Breakeven)
                activation_price = st.entry_price + self.trailing_stop_amount
                
                if st.high_price_seen >= activation_price:
                    trailing_drop = st.high_price_seen - best_bid
                    
                    if trailing_drop >= self.trailing_stop_amount:
                        logger.warning(f"📉 TRAILING STOP ({side_key}): Peak ${st.high_price_seen:.2f} -> Now ${best_bid:.2f} (Drop > ${self.trailing_stop_amount}). Selling All.")
                        await self.execute_order(side_key, st.token_id, best_bid, st.total_shares, False)
                        st.sell_shares(st.total_shares, best_bid)
                        return

            # ------------------------------------
            # C. EMERGENCY SELL (Panic Exit)
            # ------------------------------------
            # Logic: Sell 100% if Stoch > 80 OR MFI > 50 (Adjusted Settings)
            if stoch_k > self.stoch_panic_exit or mfi >= self.mfi_panic_exit:
                reason = f"Stoch {stoch_k:.1f}" if stoch_k > self.stoch_panic_exit else f"MFI {mfi:.1f}"
                logger.warning(f"🚨 EMERGENCY SELL ({side_key}): Indicator Spike ({reason})")
                await self.execute_order(side_key, st.token_id, best_bid, st.total_shares, False)
                st.sell_shares(st.total_shares, best_bid)
                return

            # ------------------------------------
            # D. TAKE PROFIT (Partials)
            # ------------------------------------
            # Logic: Sell 50% if PnL >= +25%
            if pnl_pct >= self.tp_trigger_pct and not st.taken_profit:
                sell_size = normalize_size(st.total_shares * 0.5) 
                if sell_size > 0:
                    logger.info(f"💰 TAKE PROFIT ({side_key}): +{pnl_pct*100:.1f}% hit. Selling 50%.")
                    await self.execute_order(side_key, st.token_id, best_bid, sell_size, False)
                    st.sell_shares(sell_size, best_bid)
                    st.taken_profit = True

            # ------------------------------------
            # E. TIME-BASED CHECKPOINTS
            # ------------------------------------
            time_held = time.time() - st.entry_time
            time_held_mins = time_held / 60.0

            # Momentum Check: Stoch Rising from entry?
            stoch_improving = (stoch_k > st.entry_stoch_k)

            # --- CHECKPOINT #1: T + 4 Minutes (Early Proof) ---
            if time_held_mins >= self.t4_checkpoint_min and not st.checked_t4:
                has_proof = (
                    price_gain >= self.t4_price_bump or
                    pnl_pct >= self.t4_min_pnl_pct or
                    stoch_improving
                )
            ...
            # --- CHECKPOINT #2: T + 6 Minutes (Primary Exit) ---
            if time_held_mins >= self.t6_exit_min and not st.checked_t6:
                stoch_improving = stoch_k > st.entry_stoch_k and price_gain >= 0.01
                is_working = (
                    price_gain >= self.t6_price_bump or
                    pnl_pct >= self.t6_min_pnl_pct or
                    stoch_improving
                )
    ...
    async def monitor_wss(self):
        ...
        while True:
            ...
            logger.info("=" * 70)
            logger.info("🚀 PANIC EXHAUSTION BOT STARTED (PARALLEL MODE)")
...
async def main():
    ...
    try:
        ...
        print_header("🚀 BTC 15-Minute Panic Exhaustion Bot")
