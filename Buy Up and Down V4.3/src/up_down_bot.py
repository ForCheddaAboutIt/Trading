...
class ContrarianShortBot:
    ...
    def __init__(self, settings, initial_balance=None):
    ...
    async def process_strategy(self, side_key: str, book: dict):
        ...
        if st.total_shares > 0:
            ...
            # A. INITIALIZE STOP LOSS (If not set)
            if st.stop_loss == 0.0:
                # Load Hard Stop % (default 16%)
                hard_stop_pct = float(os.getenv("STRATEGY_HARD_STOP_PERCENT", 0.16))
                st.stop_loss = st.entry_price * (1.0 - hard_stop_pct)
                
                # Ensure stop doesn't go below 0
                if st.stop_loss < 0.01: st.stop_loss = 0.01
                
                logger.info(f"🛡️ INITIAL STOP SET ({side_key}): ${st.stop_loss:.3f} (Entry ${st.entry_price:.3f} - {hard_stop_pct*100:.1f}%)")

            # ------------------------------------------------------------------
            # B. CHECK HARD STOP LOSS (Execution Priority #1)
            # ------------------------------------------------------------------
            if st.size_remaining_pct > 0.25:
                ...
                # Optional: Log that we are riding the runner without a stop
                # logger.debug(f"🏃 RUNNER MODE ({side_key}): Hard stop ignored.")
                pass
            ...
            # 3rd Scale: > 50% Profit (Target: 0.25 remaining)
            if pnl_pct >= 0.50 and st.size_remaining_pct > 0.25:
            ...
            # 2nd Scale: > 30% Profit (Target: 0.50 remaining)
            elif pnl_pct >= 0.30 and st.size_remaining_pct > 0.50:
            ...
            if self.get_time_remaining() == "CLOSED":
                ...
                try:
                    ...
                    if new_market != self.market_slug:
                        # Capture current money before resetting
                        carried_balance = self.sim_balance 
                        
                        await self.close()
                        
                        # Pass the money into the next round
                        self.__init__(self.settings, initial_balance=carried_balance) 
                        self.summary_shown = False
