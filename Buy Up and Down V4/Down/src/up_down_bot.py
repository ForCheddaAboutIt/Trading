...
class ContrarianShortBot:
    ...
    def __init__(self, settings):
        ...
        # --- Exit Settings ---
        self.trailing_stop_enabled = os.getenv("STRATEGY_TRAILING_STOP_ENABLED", "true").lower() == "true"
        self.trailing_stop_amount = float(os.getenv("STRATEGY_TRAILING_STOP_AMOUNT", 0.05))
        # HARD STOP: Exit if PnL is >= 16% (Profit) or <= -16% (Loss)
        self.hard_stop_pct = float(os.getenv("STRATEGY_HARD_STOP_PERCENT", 0.16))
    ...
    async def process_strategy(self, side_key: str, book: dict):
        ...
        if st.total_shares > 0:
            ...
            # NEW: HARD STOP (+/- 16%)
            # ------------------------------------
            # Check for Hard Stop Loss (<= -16%)
            if pnl_pct <= -self.hard_stop_pct:
                logger.warning(f"🛑 HARD STOP LOSS ({side_key}): PnL {pnl_pct*100:.1f}% hit limit (-{self.hard_stop_pct*100:.0f}%). Selling All.")
                await self.execute_order(side_key, st.token_id, best_bid, st.total_shares, False)
                st.sell_shares(st.total_shares, best_bid)
                return

            # Check for Hard Take Profit (>= 16%)
            if pnl_pct >= self.hard_stop_pct:
                logger.info(f"🎉 HARD TAKE PROFIT ({side_key}): PnL +{pnl_pct*100:.1f}% hit limit (+{self.hard_stop_pct*100:.0f}%). Selling All.")
                await self.execute_order(side_key, st.token_id, best_bid, st.total_shares, False)
                st.sell_shares(st.total_shares, best_bid)
                return
            ...
            if self.trailing_stop_enabled:
                ...
                if st.high_price_seen >= activation_price:
                    ...
                    if trailing_drop >= self.trailing_stop_amount:
                        logger.warning(f"📉 TRAILING STOP ({side_key}): Peak ${st.high_price_seen:.2f} -> Now ${best_bid:.2f}. Selling All.")
                        await self.execute_order(side_key, st.token_id, best_bid, st.total_shares, False)
                        st.sell_shares(st.total_shares, best_bid)
                        return

            # ------------------------------------
            # C. INDICATOR NORMALIZATION / PANIC EXIT
            # ------------------------------------
            # Logic: If Short (DOWN), Exit if MFI Drops < 50 (Thesis Invalidated)
            if side_key == "DOWN" and mfi < self.mfi_normalization_exit:
                logger.warning(f"🤷 MFI Normalization Exit ({mfi:.1f} < 50). Thesis Invalid.")
                await self.execute_order(side_key, st.token_id, best_bid, st.total_shares, False)
                st.sell_shares(st.total_shares, best_bid)
                return

            # Logic: If Long (UP), Exit if indicators spike high (e.g. MFI > 80)
            if side_key == "UP" and mfi > 80:
                logger.warning(f"🤷 Panic Long Normalization ({mfi:.1f} > 80). Taking Safe Exit.")
                await self.execute_order(side_key, st.token_id, best_bid, st.total_shares, False)
                st.sell_shares(st.total_shares, best_bid)
                return
