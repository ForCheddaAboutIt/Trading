...
def normalize_size(size: float, min_size: float = .01) -> float:
...
class PositionState:
    ...
    def __init__(self, token_id, side_name):
        ...
        self.moonbag_shares = 0.0  # Shares locked until market close
    ...
    def sellable_shares(self):
        return max(0.0, self.total_shares - self.moonbag_shares)
...
class ContrarianShortBot:
    ...
    def __init__(self, settings):
        ...
        # --- Exit Settings ---
        self.atr_multiplier = float(os.getenv("STRATEGY_ATR_MULTIPLIER", 3.0))
    ...
    async def check_exits(self, side_key: str, st: PositionState, current_price: float, current_rsi: float, pnl_pct: float, current_btc: float, atr_value: float) -> bool:
        """
        Consolidated Exit Logic (Priority Order):
        1. SHADOW STOP (BTC Protection) - Critical Fast Exit.
        2. SCALING - Bank safety profit at +15%.
        3. TECHNICAL EXIT - RSI / Mean Reversion.
        """
        
        # ==============================================================================
        # 1. SHADOW STOP LOGIC (PRIORITY #1) [NEW]
        # ==============================================================================
        # Logic: Fixed Stop initially. Becomes Trailing Stop if PnL >= 5%.
        
        if st.btc_stop_price > 0 and current_btc > 0 and atr_value > 0:
            
            # A. Trailing Activation (Only trail if trade is proving itself)
            if pnl_pct >= 0.05:
                if side_key == "UP":
                    # Ratchet UP (Tighten stop)
                    ...
                    # Ratchet DOWN (Tighten stop)
                    potential_new_stop = current_btc + (atr_value * self.atr_multiplier)
                    if potential_new_stop < st.btc_stop_price:
                        st.btc_stop_price = potential_new_stop

            # B. Trigger Check (Immediate Execution)
            stop_hit = False
            reason = ""
            
            if side_key == "UP" and current_btc <= st.btc_stop_price:
                stop_hit = True
                reason = f"BTC Shadow Stop Hit (${current_btc:.2f} <= ${st.btc_stop_price:.2f})"
            elif side_key == "DOWN" and current_btc >= st.btc_stop_price:
                stop_hit = True
                reason = f"BTC Shadow Stop Hit (${current_btc:.2f} >= ${st.btc_stop_price:.2f})"

            if stop_hit:
                logger.warning(f"🛑 CRITICAL EXIT ({side_key}): {reason}")
                await self.execute_order(side_key, st.token_id, current_price, st.sellable_shares, False)
                st.sell_shares(st.sellable_shares, current_price)
                return True # Position Closed

        # ==============================================================================
        # 2. SCALING LOGIC (PRIORITY #2) [EXISTING]
        # ==============================================================================
        # Bank "Safety Profit" (25% size) at +15% PnL.
        if pnl_pct >= 0.15 and st.size_remaining_pct > 0.75:
            ...
            if final_sell_size > st.sellable_shares:
                final_sell_size = st.sellable_shares
            ...
            moonbag_size = normalize_size(st.initial_size)
            
            if moonbag_size > 0:
                logger.info(f"🌑 MOONBAG TRIGGER ({side_key}): Buying 5% separate pot to hold.")
                # Execute Buy
                success = await self.execute_order(side_key, st.token_id, current_price, moonbag_size, True)
                
                if success:
                    # Mark these specific shares as locked
                    st.moonbag_shares += moonbag_size
                    
                    # Note: st.add_buy() is called inside execute_order automatically, 
                    # so st.total_shares increases. We just tracked the lock above.
                    st.add_buy(current_price, moonbag_size, 0, btc_price=current_btc) # Manually update state if needed, but execute_order usually handles it.
            
            return False # Position still open (Partial exit)

        # ==============================================================================
        # 3. PROFIT TAKING LOGIC (PRIORITY #3) [EXISTING]
        # ==============================================================================
    ...
    async def process_strategy(self, side_key: str, book: dict):
        ...
        if st.sellable_shares > 0:
            ...
            # ------------------------------------------------------------------
            is_closed = await self.check_exits(side_key, st, current_price, rsi_value, pnl_pct, current_btc, atr_value)
            if is_closed:
                return
            ...
            if time_held_mins >= self.t4_checkpoint_min and not st.checked_t4:
                ...
                if has_proof:
                    ...
                    sell_size = normalize_size(st.sellable_shares * 0.5)
                    if sell_size > 0:
            ...
            if time_held_mins >= self.t6_exit_min and not st.checked_t6:
                ...
                if is_working:
                    ...
                    await self.execute_order(side_key, st.token_id, best_bid, st.sellable_shares, False)
                    st.sell_shares(st.sellable_shares, best_bid)
    ...
    async def monitor_wss(self):
        ...
        while True:
            if self.get_time_remaining() == "CLOSED":
                # ... [Keep your existing Market Discovery/Reset logic here] ...
                # (I am omitting the long reset block for brevity, keep it exactly as you have it)
            ...
            try:
                async for asset_id, event_type in client.run():
                    ...
                    now = time.time()
                    # [REMOVED] The top-level throttle. We want to parse EVERY tick.
                    ...
                    # ==================================================================
                    # ⚡ FAST SAFETY CHECKS (Runs on every tick)
                    # ==================================================================
                    panic_triggered = False

                    for side in ["UP", "DOWN"]:
                        st = self.strategies[side]
                        if st.sellable_shares <= 0: continue 

                        triggered = False
                        reason = ""
                        exit_price = 0.0

                        current_opt_price = self.latest_up_bid if side == "UP" else self.latest_down_bid

                        # A. BTC Shadow Stop
                        if self.latest_btc_price > 0 and st.btc_stop_price > 0:
                            if side == "UP" and self.latest_btc_price <= st.btc_stop_price:
                                triggered = True
                                reason = f"BTC SHADOW STOP (${self.latest_btc_price:,.2f} <= ${st.btc_stop_price:,.2f})"
                            elif side == "DOWN" and self.latest_btc_price >= st.btc_stop_price:
                                triggered = True
                                reason = f"BTC SHADOW STOP (${self.latest_btc_price:,.2f} >= ${st.btc_stop_price:,.2f})"

                        # B. Option Hard Stop
                        if not triggered and st.stop_loss > 0 and current_opt_price > 0:
                            if current_opt_price <= st.stop_loss:
                                triggered = True
                                reason = f"OPTION HARD STOP (${current_opt_price:.3f} <= ${st.stop_loss:.3f})"

                        if triggered:
                            logger.warning(f"⚡ CRITICAL FAST EXIT ({side}): {reason}")
                            exit_price = current_opt_price
                            await self.execute_order(side, st.token_id, exit_price, st.sellable_shares, False)
                            st.sell_shares(st.sellable_shares, exit_price)
                            panic_triggered = True # Mark to skip strategy
                    
                    if panic_triggered:
                        continue # Skip strategy cycle if we just panic sold

                    # ==================================================================
                    # 2. NORMAL STRATEGY CYCLE (Throttled)
                    # ==================================================================
                    # Only run heavy logic every 0.1s
                    if (now - last_eval) > 0.1: 
                        last_eval = now # Update time ONLY here
