...
class ContrarianShortBot:
    ...
    def __init__(self, settings):
        ...
        self.stop_buffer_pct = float(os.getenv("STRATEGY_TRAILING_STOP_AMOUNT", 0.15))
    ...
    async def process_strategy(self, side_key: str, book: dict):
        ...
        if can_buy:
            ...
            # --- B. PANIC LONG (UP) - Restored for capability ---
            elif side_key == "UP":
                # Logic: MFI <= 8 AND Stoch <= 20 (Typical Panic Long parameters)
        ...
        if st.total_shares > 0:
            ...
            if side_key == "DOWN": # Invert PnL logic for Short? 
                # NOTE: Polymarket "Short" is just buying "NO" shares. 
                # If you buy NO at 0.40 and it goes to 0.60, you made profit. 
                # So standard (current - entry) / entry works for both UP and DOWN tokens.
                pass 

            # ------------------------------------------------------------------
            # A. INITIALIZE STOP LOSS (If not set)
            # ------------------------------------------------------------------
            stop_buffer = st.entry_price * self.stop_buffer_pct
            if st.stop_loss == 0.0:
                st.stop_loss = st.entry_price - stop_buffer
                # Ensure stop doesn't go below 0
                if st.stop_loss < 0.01: st.stop_loss = 0.01
                
                logger.info(f"🛡️ INITIAL STOP SET ({side_key}): ${st.stop_loss:.3f} (Entry ${st.entry_price:.3f} - $0.06)")
            ...
            # ------------------------------------------------------------------
            # C. RSI EXHAUSTION EXIT (Emergency Exit)
            # ------------------------------------------------------------------
            is_rsi_exit = False
            if side_key == "UP" and rsi_value > self.rsi_exit_long:
                logger.warning(f"🔥 RSI EXHAUSTION (UP): {rsi_value:.1f} > {self.rsi_exit_long}. Exiting.")
                is_rsi_exit = True
            elif side_key == "DOWN" and rsi_value < self.rsi_exit_short:
                # Note: If we hold "NO" shares (DOWN), we benefit if the underlying asset drops.
                # If underlying BTC drops, RSI drops.
                # So we want to exit our "Short" (NO shares) when RSI is Oversold (exhausted).
                logger.warning(f"🔥 RSI EXHAUSTION (DOWN): {rsi_value:.1f} < {self.rsi_exit_short}. Exiting.")
                is_rsi_exit = True
            ...
            if pnl_pct >= 0.15 and not st.is_breakeven_set:
                st.stop_loss = max(st.stop_loss, st.entry_price) # Move to entry (never lower)
                st.is_breakeven_set = True
                logger.info(f"🛡️ BREAKEVEN ACTIVATED ({side_key}): Stop moved to ${st.stop_loss:.3f}")

            # ------------------------------------------------------------------
            # E. ATR TRAILING STOP RATCHET
            # ------------------------------------------------------------------
            new_stop = current_price - stop_buffer
            
            if new_stop > st.stop_loss:
                st.stop_loss = new_stop

            # ------------------------------------------------------------------
            # F. PARTIAL POSITION SCALING (Liquidity Harvesting)
            # ------------------------------------------------------------------
            # Targets: 15% (sell 25%), 25% (sell 25%), 40% (sell 25%) -> Leave 25% runner
            
            # Check largest target first to handle massive candles (Gaps)
            scale_size = 0.0
            
            # 3rd Scale: > 40% Profit (Target: 0.25 remaining)
            if pnl_pct >= 0.40 and st.size_remaining_pct > 0.25:
                # We want to go from whatever we have down to 0.25
                # Ex: If we have 0.75, we sell 0.50. If we have 0.50, we sell 0.25.
                pct_to_sell = st.size_remaining_pct - 0.25
                scale_size = st.initial_size * pct_to_sell
                st.size_remaining_pct = 0.25
                logger.info(f"💰 SCALING LEVEL 3 ({side_key}): +40% Hit. Selling down to runner.")

            # 2nd Scale: > 25% Profit (Target: 0.50 remaining)
            elif pnl_pct >= 0.25 and st.size_remaining_pct > 0.50:
                pct_to_sell = st.size_remaining_pct - 0.50
                scale_size = st.initial_size * pct_to_sell
                st.size_remaining_pct = 0.50
                logger.info(f"💰 SCALING LEVEL 2 ({side_key}): +25% Hit. Locking profit.")

            # 1st Scale: > 15% Profit (Target: 0.75 remaining)
            elif pnl_pct >= 0.15 and st.size_remaining_pct > 0.75:
                pct_to_sell = st.size_remaining_pct - 0.75
                scale_size = st.initial_size * pct_to_sell
                st.size_remaining_pct = 0.75
                logger.info(f"💰 SCALING LEVEL 1 ({side_key}): +15% Hit. Harvesting initial liquidity.")
            
            if scale_size > 0:
                # Normalize and ensure we don't sell more than we own (rounding errors)
            ...
            # ------------------------------------------------------------------
            # G. TIME CHECKPOINTS (Safety Net)
            # ------------------------------------------------------------------
            # (Keep your existing T+4 and T+6 logic here as a final fallback 
            #  if the market goes flat and RSI/Stop doesn't trigger)
            # ... [Paste your existing T+4 / T+6 Code Block here] ...
